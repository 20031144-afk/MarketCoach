...
class RateLimitException extends AnalysisException {
  RateLimitException()
      : super('Daily analysis limit reached. Try again tomorrow.',
            statusCode: 429);
}

class ServiceUnavailableException extends AnalysisException {
  ServiceUnavailableException()
      : super('AI analysis service is temporarily unavailable.',
            statusCode: 503);
}
...
class ClaudeAnalysisService {
  ...
  Future<EnhancedAIAnalysis> getStructuredAnalysis(String symbol) async {
    if (!APIConfig.isConfigured) {
      throw AnalysisException(APIConfig.configurationError);
    }
  ...
  String _buildJsonPrompt(StockData data) {
    ...
    buffer.writeln('You are a financial analyst. Analyze ${data.symbol} and respond ONLY with a JSON object - no explanation, no markdown, no code fences, just raw JSON.');
    buffer.writeln();
    ...
    if (data.changePercent != null) {
      buffer.writeln('Today Change: ${data.changePercent!.toStringAsFixed(2)}%');
    }
    if (data.dayHigh != null && data.dayLow != null) {
      buffer.writeln('Day Range: \$${data.dayLow!.toStringAsFixed(2)} - \$${data.dayHigh!.toStringAsFixed(2)}');
    }
    if (data.fiftyTwoWeekHigh != null && data.fiftyTwoWeekLow != null) {
      buffer.writeln('52-Week Range: \$${data.fiftyTwoWeekLow!.toStringAsFixed(2)} - \$${data.fiftyTwoWeekHigh!.toStringAsFixed(2)}');
      final pos = data.fiftyTwoWeekPosition;
      if (pos != null) buffer.writeln('52-Week Position: ${pos.toStringAsFixed(0)}%');
    }
    if (data.volume != null) {
    ...
    if (data.news.isNotEmpty) {
      ...
      for (final n in data.news.take(5)) {
        buffer.writeln('- ${n.title}${n.source != null ? ' (${n.source})' : ''}');
      }
    }

    buffer.writeln();
    buffer.writeln('Respond with EXACTLY this JSON structure and nothing else:');
    buffer.writeln('''
{
  ...
  "price_target": <realistic 7-day price target as a number, e.g. 185.50>,
  "price_low": <realistic 7-day downside as a number>,
  "price_high": <realistic 7-day upside as a number>,
  ...
  EnhancedAIAnalysis _parseJsonResponse(
      String rawJson, double currentPrice, String symbol) {
    try {
      ...
      // factors
      final bullish = (json['bullish_factors'] as List?)
              ?.map((e) => e.toString())
              .toList() ??
          [];
      final bearish = (json['bearish_factors'] as List?)
              ?.map((e) => e.toString())
              .toList() ??
          [];

      // price target
      final targetVal = (json['price_target'] as num?)?.toDouble();
      final lowVal = (json['price_low'] as num?)?.toDouble();
      final highVal = (json['price_high'] as num?)?.toDouble();
      ...
      throw AnalysisException('Failed to parse Claude response: $e\n\nRaw: $rawJson');
    }
  }
    ...
    try {
      ...
      switch (response.statusCode) {
        ...
        default:
          ...
          throw AnalysisException('Claude API error (${response.statusCode}): $msg',
